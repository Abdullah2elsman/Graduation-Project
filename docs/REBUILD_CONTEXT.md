...
Architecture, product decisions, the canonical database schema, and the authentication contract are locked. Laravel, Angular, Flask, Docker, MySQL, and Mailpit foundations exist; Phase 1B is committed at `d474d70`, and the auth contract at `eaa77d3`. Phase 1C.2 now provides Sanctum 4.3.3, stateful `/api` middleware, database-session/cookie/CORS configuration, a Docker-aware Angular dev proxy, centralized HttpClient/XSRF infrastructure, and foundation tests. No auth business endpoint exists yet. The exact next step is Phase 1C.3: email-only login, logout, and `GET /api/auth/me` as defined in `docs/auth/AUTH_CONTRACT.md` and `ROADMAP.md`.
